...
- 확장함수
예)
장황한 표현: StringUtils.startWithA(str)
간결한 표헌: str.startWithA()

- 중위 함수 호출
예) 원래 점과 괄호를 꼭 쳐야하지만 매개변수가 하나인 함수 같은 경우에는
왼쪽와 오른쪽 사이에 함수 이름을 쓰는것으로 함수 호출
장황한 표현: "AB".concat("DE")
간결한 표헌: "FGH" concat "IJK"

- 람다식 활용
예) 람다식 같은 경우에는 어떤 함수가 또 다른 함수를 매개변수로 받는 그 매개변수가 가장 마지막일때
...
수신 객체 람다를 활용하면 함수 안에 this 라는 키워드로 해당 객체를 가리키게 되면서 this를 생략하고 호출할 수 있다
장황한 표현:
val data = Data()
function {
    data.call()
